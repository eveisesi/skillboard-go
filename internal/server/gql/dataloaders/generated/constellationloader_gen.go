...
type ConstellationLoader struct {
	...
	cache map[uint]*skillz.Constellation
...
func (l *ConstellationLoader) Load(ctx context.Context, key uint) (*skillz.Constellation, error) {
...
func (l *ConstellationLoader) LoadThunk(ctx context.Context, key uint) func() (*skillz.Constellation, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*skillz.Constellation, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *ConstellationLoader) LoadAll(ctx context.Context, keys []uint) ([]*skillz.Constellation, []error) {
	results := make([]func() (*skillz.Constellation, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(ctx, key)
	}

	constellations := make([]*skillz.Constellation, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *ConstellationLoader) LoadAllThunk(ctx context.Context, keys []uint) func() ([]*skillz.Constellation, []error) {
	results := make([]func() (*skillz.Constellation, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(ctx, key)
	}
...
func (l *ConstellationLoader) Prime(key uint, value *skillz.Constellation) bool {
	l.mu.Lock()
	var found bool
...
func (l *ConstellationLoader) unsafeSet(key uint, value *skillz.Constellation) {
	if l.cache == nil {
		l.cache = map[uint]*skillz.Constellation{}
	}
	l.cache[key] = value
}
...
func (b *constellationLoaderBatch) keyIndex(ctx context.Context, l *ConstellationLoader, key uint) int {
...
func (b *constellationLoaderBatch) startTimer(ctx context.Context, l *ConstellationLoader) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *constellationLoaderBatch) end(ctx context.Context, l *ConstellationLoader) {
