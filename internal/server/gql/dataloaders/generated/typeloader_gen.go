...
func (l *TypeLoader) Load(ctx context.Context, key uint) (*skillz.Type, error) {
...
func (l *TypeLoader) LoadThunk(ctx context.Context, key uint) func() (*skillz.Type, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*skillz.Type, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *TypeLoader) LoadAll(ctx context.Context, keys []uint) ([]*skillz.Type, []error) {
	results := make([]func() (*skillz.Type, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(ctx, key)
	}

	types := make([]*skillz.Type, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *TypeLoader) LoadAllThunk(ctx context.Context, keys []uint) func() ([]*skillz.Type, []error) {
	results := make([]func() (*skillz.Type, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(ctx, key)
	}
...
func (l *TypeLoader) Prime(key uint, value *skillz.Type) bool {
	l.mu.Lock()
	var found bool
...
func (l *TypeLoader) unsafeSet(key uint, value *skillz.Type) {
	if l.cache == nil {
		l.cache = map[uint]*skillz.Type{}
	}
	l.cache[key] = value
}
...
func (b *typeLoaderBatch) keyIndex(ctx context.Context, l *TypeLoader, key uint) int {
...
func (b *typeLoaderBatch) startTimer(ctx context.Context, l *TypeLoader) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *typeLoaderBatch) end(ctx context.Context, l *TypeLoader) {
