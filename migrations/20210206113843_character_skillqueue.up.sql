CREATE TABLE `character_skillqueue` (
    ...
    CONSTRAINT `character_skillqueue_character_id_foreign` FOREIGN KEY (`character_id`) REFERENCES `skillboard`.`users` (`character_id`) ON UPDATE CASCADE ON DELETE CASCADE
